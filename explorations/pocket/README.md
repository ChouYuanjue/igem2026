# Pocket Exploration for EnzymeCAGE
...
This exploration does not train EnzymeCAGE and does not modify the EnzymeCAGE
...
python explorations/pocket/runners/generate_baseline_configs.py \
  ...
  --output_dir explorations/pocket/configs/generated
```
...
```bash
bash scripts/inspect_enzymecage_assets.sh
bash scripts/inspect_enzymecage_configs.sh
```

Inspect EnzymeCAGE:

```bash
bash scripts/inspect_enzymecage.sh
```
...
python explorations/pocket/runners/run_compare_baselines.py \
  --experiment_config explorations/pocket/configs/generated/demo_p2rank_topk_max.yaml
```

Run the full matrix:

```bash
bash scripts/run_real_pocket_experiment.sh
```
