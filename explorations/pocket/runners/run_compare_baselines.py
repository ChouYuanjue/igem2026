...
class ExperimentRunner:
    ...
    def run_pocket_adapter(self, source: str, pair_csv: Path, top_k: int, sub_run_id: str) -> tuple[Path | None, dict[str, Any] | None]:
        ...
        if source == "p2rank":
            command = [
                sys.executable,
                str(PROJECT_ROOT / "explorations/pocket/adapters/p2rank_multi_to_enzymecage.py"),
                "--input_csv",
                str(pair_csv),
                "--structure_dir",
                str(structure_dir),
            ...
            command = [
                sys.executable,
                str(PROJECT_ROOT / "explorations/pocket/adapters/fpocket_to_enzymecage.py"),
                "--input_csv",
                str(pair_csv),
                "--structure_dir",
                str(structure_dir),
            ...
            command = [
                sys.executable,
                str(PROJECT_ROOT / "explorations/pocket/adapters/official_precomputed_to_enzymecage.py"),
                "--input_csv",
                str(pair_csv),
                "--pocket_dir",
                str(pocket_dir),
    ...
    def extract_pockets(self, pair_csv: Path) -> Path | None:
        ...
        if source == "p2rank+fpocket" or selection == "union_topk":
            ...
            command = [
                sys.executable,
                str(PROJECT_ROOT / "explorations/pocket/adapters/merge_pocket_manifests.py"),
                "--manifest_csvs",
    ...
    def run_aggregation(self, pocket_prediction_csv: Path, manifest_csv: Path) -> Path:
        ...
        command = [
            sys.executable,
            str(PROJECT_ROOT / "explorations/pocket/analysis/aggregate_pocket_scores.py"),
            "--prediction_csv",
    ...
    def run_evaluation(self, aggregated_csv: Path) -> None:
        ...
        command = [
            sys.executable,
            str(PROJECT_ROOT / "explorations/pocket/analysis/evaluate_topk.py"),
            "--prediction_csv",
            str(aggregated_csv),
            "--label_csv",
            str(label_csv),
    ...
    def build_derived_smallset(self) -> Path | None:
        ...
        command = [
            sys.executable,
            str(PROJECT_ROOT / "explorations/pocket/adapters/build_official_smallset.py"),
            "--source_dataset",
            str(source_dataset),
            "--enzymecage_root",
            str(enzymecage_root),
        ...
        if status != "derived_smallset_completed":
            ...
            command = [
                sys.executable,
                str(PROJECT_ROOT / "explorations/pocket/adapters/download_alphafold_structures.py"),
                "--input_csv",
                str(base_pair_csv),
                "--output_root",
