...
CACHE=ROOT/'data/terpene_marts_adaptation';PG=ROOT/'data/terpene_multiresolution_protein_geometry_v4';RG=ROOT/'data/terpene_multiresolution_reaction_geometry_v1';OUT=ROOT/'results/terpene_stable_levelset_correspondence_dev_v1';BUD=(3,10,20)
...
def rank_lex(primary,secondary,ids,pos,budgets):
 primary=np.asarray(primary,dtype=np.float64);secondary=np.asarray(secondary,dtype=np.float64);ids_arr=np.asarray(ids)
 base=np.argsort(primary,kind='stable')
 scale=max(1.0,float(np.max(np.abs(primary[np.isfinite(primary)]))))
 tol=64.0*np.finfo(np.float64).eps*scale
 ordered=[];start=0
 while start<len(base):
  ref=primary[base[start]];end=start+1
  while end<len(base) and abs(primary[base[end]]-ref)<=tol:end+=1
  group=base[start:end]
  local=np.argsort(ids_arr[group],kind='stable')
  ordered.extend(group[local].tolist());start=end
 ranked=[ids[i] for i in ordered];positions=np.asarray([k+1 for k,v in enumerate(ranked) if v in pos],int);best=int(positions.min()) if len(positions) else None
 out={'candidate_count':len(ranked),'n_positives':len(pos),'best_positive_rank':best,'reciprocal_rank':1/best if best else 0.0,'mean_positive_rank':float(positions.mean()) if len(positions) else None,'numeric_level_tolerance':tol}
 for k in budgets:
...
def main():
 ...
 summary={'version':'terpene-stable-levelset-correspondence-dev-v1','partition':'development_only','ranking':'machine-stable correspondence-defect level sets; candidates within 64 machine eps of the level reference are treated as numerically identical and ordered only by candidate id for a neutral reproducible baseline','interpretation':'neutral control for evaluating whether any biological secondary observable adds information inside underdetermined defect level sets','parameter_selection':'none','characteristic_lengths':{'reaction':lr,'protein':le},'all_overlaps_zero':bool(((sp.protein_overlap==0)&(sp.reaction_overlap==0)).all()),'metrics':met.to_dict('records')}
