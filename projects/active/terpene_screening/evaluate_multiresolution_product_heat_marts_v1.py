...
REACTION_GEOM=ROOT/'data/terpene_multiresolution_reaction_geometry_v1'
OUT=ROOT/'results/terpene_multiresolution_product_heat_dev_v1'
BUDGETS=(3,10,20)
...
def random_walk_generator(w: csr_matrix) -> csr_matrix:
    ...
    inv=np.zeros_like(degree); positive=degree>0; inv[positive]=1.0/degree[positive]
    p=(diags(inv)@w).tocsr()
    q=p.tolil()
...
def heat_joint(relation: np.ndarray, reaction_graph: csr_matrix, protein_graph: csr_matrix) -> np.ndarray:
    qr=random_walk_generator(reaction_graph)
    qe=random_walk_generator(protein_graph)
    left=expm_multiply(qr,np.asarray(relation,dtype=np.float64))
...
def main() -> None:
    ...
    methods={
      ...
      'all_information':(reaction_v1,protein_v4),
    }
    ...
    summary={
      'version':'terpene-multiresolution-product-heat-dev-v1','partition':'development_only','development_rule':'protein_fold==4 or reaction_fold==4',
      'test_rule':'same cell and protein_seen==false and reaction_seen==false','methods':list(methods),
      'operator':'J = exp(P_R-I) mu exp(P_E-I)^T at unit continuous-time diffusion; isolated rows use zero generator',
      'parameter_selection':'none; no grid search, learned score fusion, negatives, or frozen partition labels',
