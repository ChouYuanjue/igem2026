...
CACHE=ROOT/'data/terpene_marts_adaptation'; PG=ROOT/'data/terpene_multiresolution_protein_geometry_v4'; RG=ROOT/'data/terpene_multiresolution_reaction_geometry_v2'; OUT=ROOT/'results/terpene_product_geodesic_dev_v1'; BUD=(3,10,20)
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def main():
 ...
 pg0=one_view(np.load(PG/'global_esmc_chordal_distance.npy'),np.load(PG/'global_esmc_available.npy'));rg0=one_view(np.load(RG/'drfp_chordal_distance.npy'),np.load(RG/'drfp_available.npy'));pgM=load_npz(PG/'partial_pullback_affinity.npz');rgM=load_npz(RG/'partial_pullback_affinity.npz')
 Dp0,cp0=energy_geodesic(pg0);Dr0,cr0=energy_geodesic(rg0);DpM,cpM=energy_geodesic(pgM);DrM,crM=energy_geodesic(rgM)
 methods={'global_only':(Dr0,Dp0),'protein_multires_v4':(Dr0,DpM),'reaction_center_v2':(DrM,Dp0),'all_information_v2':(DrM,DpM)}
 rows=[];spl=[]
 for pf in range(5):
  for rf in range(5):
 ...
 summary={'version':'terpene-product-geodesic-dev-v1','partition':'development_only','field':'F(r,e)=-D_Omega(r,e)^2 where D_Omega is exact Cartesian-product geodesic distance to the fold-training positive set and D_product^2=d_R^2+d_E^2','edge_length':'sqrt(-log(self-tuned pullback affinity)); numerical floor only at 1e-12 energy','parameter_selection':'none; no diffusion time, temperature, source strength, score fusion, negative pairs, or frozen labels','components':{'protein_global':cp0,'reaction_global':cr0,'protein_multires':cpM,'reaction_multires':crM},'all_overlaps_zero':bool(((sp.protein_overlap==0)&(sp.reaction_overlap==0)).all()),'input_sha256':{'split':sha(CACHE/'marts_pair_folds.csv'),'protein_geometry':sha(PG/'manifest.json'),'reaction_geometry':sha(RG/'manifest.json')},'metrics':met.to_dict('records')};(OUT/'summary.json').write_text(json.dumps(summary,indent=2)+'\n');print(met.to_string(index=False));print(json.dumps({k:v for k,v in summary.items() if k!='metrics'},indent=2))
