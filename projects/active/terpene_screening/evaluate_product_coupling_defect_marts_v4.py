...
def coupling_defect_field(train,Dr2,Dp2,ri,pi,nR,nE):
    # Joint distance to the sparse biochemical correspondence set Omega.
    joint=np.full((nR,nE),np.inf,dtype=np.float64)
    train_pairs=[(ri[str(x.rhea_id)],pi[str(x.Entry)]) for x in train.itertuples(index=False)]
    for rr,ee in train_pairs:
        joint=np.minimum(joint,Dr2[:,rr,None]+Dp2[:,ee][None,:])
    # Distances to the marginal supports discard mere familiarity/coverage effects.
    r_support=np.unique([x[0] for x in train_pairs]); e_support=np.unique([x[1] for x in train_pairs])
    marg_r=np.min(Dr2[:,r_support],axis=1)
    marg_e=np.min(Dp2[:,e_support],axis=1)
    defect=joint-marg_r[:,None]-marg_e[None,:]
    # The expression is nonnegative analytically; clip only floating roundoff.
 ...
 for pf in range(5):
  for rf in range(5):
   ...
   for method,(Dr,Dp) in methods.items():
    Dr2=Dr*Dr;Dp2=Dp*Dp; defect=coupling_defect_field(train,Dr2,Dp2,ri,pi,len(rids),len(pids)); F=-defect
