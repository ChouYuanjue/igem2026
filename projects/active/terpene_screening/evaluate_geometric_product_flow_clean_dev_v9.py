...
"""Clean-dev v9 evaluator with unit-time continuous heat mollification on one reaction-protein product manifold.
...
from projects.active.terpene_screening.geometric_pair_measure import (
    anisotropic_product_heat_pushforward,
)
...
OUT = ROOT / "results/geometric_product_flow_clean_dev_v9"
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        if protein_graphs:
            transported_measure = anisotropic_product_heat_pushforward(
    ...
    summary = {
        "method": "continuous_product_heat_charbonnier_bregman_field_v9",
        "fold": fold,
        ...
        "positive_measure_transport": "unit-time heat semigroup exp(P-I) on the full Cartesian product conductance graph, where P is the source-normalized no-self-loop Markov operator built from reaction-axis and protein-axis F0-dependent Charbonnier conductances; axis-exchange invariant and includes all product paths via the Poisson expansion",
