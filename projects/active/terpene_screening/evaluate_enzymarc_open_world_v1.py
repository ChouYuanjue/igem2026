...
SUPPORT=ROOT/'results/enzymarc_open_world_v1/support'; FEATURES=ROOT/'results/enzymarc_open_world_v1/esmc_features'; OUT=ROOT/'results/enzymarc_open_world_v1/catalyst_primary_source'
...
def main():
    ...
    parents=pd.read_csv(SUPPORT/'parents.csv',dtype=str).fillna(''); decoys=pd.read_csv(SUPPORT/'decoys.csv',dtype=str).fillna(''); rel=pd.read_csv(SUPPORT/'parent_reaction_relations.csv',dtype=str).fillna('')
    ...
    summary={'status':'evaluated_frozen_primary_source','protocol_sha256':sha(PROTOCOL),'model_checkpoint_dir':str(MODEL),'model_checkpoint_sha256':sha(next((MODEL/'models').glob('production*.pt'))),'feature_manifest_sha256':sha(FEATURES/'manifest.json'),'per_category':cats,'micro_all_decoys':micro,'selection_allowed':False,'threshold_tuning_used':False,'lambdarank_pair_score_fabricated':False}
