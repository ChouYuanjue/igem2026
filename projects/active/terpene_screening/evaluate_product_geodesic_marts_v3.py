...
CACHE=ROOT/'data/terpene_marts_adaptation'; PG=ROOT/'data/terpene_multiresolution_protein_geometry_v4'; RG=ROOT/'data/terpene_multiresolution_reaction_geometry_v2'; HG=ROOT/'data/terpene_hierarchical_factor_geometry_v1'; OUT=ROOT/'results/terpene_product_geodesic_dev_v3'; BUD=(3,10,20)
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def energy_geodesic(w):
 ...
 d=np.asarray(d,dtype=np.float64); mask=np.isfinite(d)&(~np.eye(len(d),dtype=bool)); scale=float(np.sqrt(np.mean(np.square(d[mask])))) if np.any(mask) else 1.0; return d/scale,int(ncomp),scale
def one_view(d,a):
...
def main():
 ...
 Dp0,cp0,sp0=energy_geodesic(pg0);Dr0,cr0,sr0=energy_geodesic(rg0);DpH,cpH,spH=energy_geodesic(pgH);DrH,crH,srH=energy_geodesic(rgH)
 ...
 summary={'version':'terpene-product-geodesic-dev-v3','partition':'development_only','field':'F(r,e)=-D_Omega(r,e)^2 with D_product^2=(d_R/l_R)^2+(d_E/l_E)^2 and l_M equal to the label-free RMS all-pairs intrinsic geodesic distance of factor M','edge_length':'sqrt(-log(self-tuned affinity)); each factor is rescaled to unit RMS intrinsic geodesic length before the canonical Cartesian product; numerical floor only at 1e-12 energy','parameter_selection':'none; factor characteristic lengths are label-free RMS intrinsic geodesic distances, not tuned weights; no diffusion time, temperature, source strength, score fusion, negative pairs, or frozen labels','components':{'protein_global':cp0,'reaction_global':cr0,'protein_hierarchical':cpH,'reaction_hierarchical':crH},'factor_characteristic_lengths':{'protein_global':sp0,'reaction_global':sr0,'protein_hierarchical':spH,'reaction_hierarchical':srH},'all_overlaps_zero':bool(((sp.protein_overlap==0)&(sp.reaction_overlap==0)).all()),'input_sha256':{'split':sha(CACHE/'marts_pair_folds.csv'),'protein_geometry':sha(PG/'manifest.json'),'reaction_geometry':sha(RG/'manifest.json'),'hierarchical_geometry':sha(HG/'manifest.json')},'metrics':met.to_dict('records')};(OUT/'summary.json').write_text(json.dumps(summary,indent=2)+'\n');print(met.to_string(index=False));print(json.dumps({k:v for k,v in summary.items() if k!='metrics'},indent=2))
