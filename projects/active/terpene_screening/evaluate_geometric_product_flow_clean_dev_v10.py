...
OUT = ROOT / "results/geometric_product_flow_clean_dev_v10"
...
def _project_positive_vertices_to_chart(
    ...
    *,
    ...
    """Restrict positive atoms with the same intrinsic protein metric as the graph.

    ESM-C and observed CLIP coordinates are not averaged with a fixed modality
    weight. Their self-tuning local energies receive the same perplexity-
    contraction metric precision used to construct ``g_E`` itself. Missing CLIP
    structure simply removes that coordinate. The returned rows are a geometric
    partition of unity, so every empirical positive atom preserves unit mass.
    """
    ...
    if len(chart) == 0:
        raise ValueError("protein chart must not be empty")
    if len(chart) == 1:
        return np.ones((len(obs), 1), dtype=np.float64)

    with torch.no_grad():
        ot = F.normalize(
            torch.as_tensor(np.asarray(esmc[obs], dtype=np.float32).copy(), device=device),
            dim=1,
        )
        ct = F.normalize(
            torch.as_tensor(np.asarray(esmc[chart], dtype=np.float32).copy(), device=device),
            dim=1,
        )
        cross_esmc_similarity = (ot @ ct.T).cpu().numpy().astype(np.float64)
        chart_esmc_similarity = (ct @ ct.T).cpu().numpy().astype(np.float64)
    cross_esmc_distance = np.sqrt(
        np.maximum(2.0 - 2.0 * np.clip(cross_esmc_similarity, -1.0, 1.0), 0.0)
    )
    chart_esmc_distance = np.sqrt(
        np.maximum(2.0 - 2.0 * np.clip(chart_esmc_similarity, -1.0, 1.0), 0.0)
    )

    obs_clip = clip_lookup[obs]
    chart_clip = clip_lookup[chart]
    obs_structure_available = obs_clip >= 0
    chart_structure_available = chart_clip >= 0
    cross_clip_distance = np.zeros((len(obs), len(chart)), dtype=np.float64)
    chart_clip_distance = np.zeros((len(chart), len(chart)), dtype=np.float64)
    oi = np.flatnonzero(obs_structure_available)
    cj = np.flatnonzero(chart_structure_available)
    if len(oi) and len(cj):
        with torch.no_grad():
            ...
            cross_clip_similarity = (op @ cp.T).cpu().numpy().astype(np.float64)
            chart_clip_similarity = (cp @ cp.T).cpu().numpy().astype(np.float64)
        cross_clip_distance[np.ix_(oi, cj)] = np.sqrt(
            np.maximum(2.0 - 2.0 * np.clip(cross_clip_similarity, -1.0, 1.0), 0.0)
        )
        chart_clip_distance[np.ix_(cj, cj)] = np.sqrt(
            np.maximum(2.0 - 2.0 * np.clip(chart_clip_similarity, -1.0, 1.0), 0.0)
        )

    weights, _ = resolution_product_partition(
        [cross_esmc_distance, cross_clip_distance],
        [chart_esmc_distance, chart_clip_distance],
        [np.ones(len(obs), dtype=bool), obs_structure_available],
        [np.ones(len(chart), dtype=bool), chart_structure_available],
    )
    return weights
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        # The protein factor now follows exactly the same metric principle as
        # the reaction factor. ESM-C is a complete coordinate; CLIP is an
        # observed structural coordinate. Their local metric precisions are
        # inferred from perplexity contraction, not assigned 0.5/0.5 weights.
        with torch.no_grad():
            esmc_local = F.normalize(
                torch.as_tensor(
                    np.asarray(esmc[candidates], dtype=np.float32).copy(), device=device
                ),
                dim=1,
            )
            esmc_similarity = (esmc_local @ esmc_local.T).cpu().numpy().astype(np.float64)
        esmc_distance = np.sqrt(
            np.maximum(2.0 - 2.0 * np.clip(esmc_similarity, -1.0, 1.0), 0.0)
        )
        clip_positions = clip_lookup[candidates]
        structure_available = clip_positions >= 0
        clip_distance = np.zeros((domain_size, domain_size), dtype=np.float64)
        ...
        if len(observed) >= 2:
            ...
            with torch.no_grad():
                ...
                observed_similarity = (cpt @ cpt.T).cpu().numpy().astype(np.float64)
            clip_distance[np.ix_(observed, observed)] = np.sqrt(
                np.maximum(2.0 - 2.0 * np.clip(observed_similarity, -1.0, 1.0), 0.0)
            )
        ...
        ambient_protein_affinity, protein_kernel_info = resolution_product_affinity(
            [esmc_distance, clip_distance],
            [np.ones(domain_size, dtype=bool), structure_available],
        )
        ...
        diagnostics.append(
            {
                ...
                "protein_metric_mean_view_resolution": json.dumps(protein_kernel_info.get("mean_view_resolution", [])),
                "protein_structure_observed_nodes": int(structure_available.sum()),
    ...
    summary = {
        "method": "fully_resolution_product_metric_charbonnier_bregman_field_v10",
        "fold": fold,
        ...
        "protein_geometry": "one resolution-product metric on ESM-C and observed CLIP coordinates; each local coordinate precision is exp(KL(local self-tuning kernel || uniform))-1, missing structure omits that coordinate, followed by exact t=1 diffusion-conformal conductance on the resulting atlas edges",
        "flow_strength": 1.0,
