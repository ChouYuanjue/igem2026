...
OUT = ROOT / "results/product_field_r2e_section_probe_v1"
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        candidate_pos = {int(row): int(i) for i, row in enumerate(candidates)}
        positive_nodal = []
        for pid in positives[q]:
            rr = int(pindex[pid])
            if rr in candidate_pos:
                ci = candidate_pos[rr]
                positive_nodal.append({
                    "protein_id": str(pid),
                    "prior_score": float(prior[0, ci]),
                    "flow_score": float(flowed[0, ci]),
                    "delta": float(flowed[0, ci] - prior[0, ci]),
                    "chart_index": int(ci),
                })
    ...
    summary = {
        "method": "r2e_section_consistency_probe_v1",
        "fold": fold,
