...
"""Clean-dev v7 evaluator with one axis-symmetric Markov mollifier on the Cartesian product manifold.
...
OUT = ROOT / "results/geometric_product_flow_clean_dev_v7"
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        if protein_graphs:
            transported_measure = anisotropic_product_pushforward(
                prior,
                empirical_measure,
                reaction_graph,
                protein_graphs[0],
            ...
            # Degenerate reaction-only ablation is still one factor of the same
            # Cartesian product and is handled by a zero protein graph.
            transported_measure = anisotropic_product_pushforward(
                prior, empirical_measure, reaction_graph,
                csr_matrix((prior.shape[1], prior.shape[1]), dtype=np.float32),
                scale=transport_scale, include_identity=True,
            )
    ...
    summary = {
        "method": "axis_symmetric_product_measure_charbonnier_bregman_field_v7",
        "fold": fold,
        ...
        "positive_measure_transport": "one mass-conserving lazy Markov push-forward on the full Cartesian product graph: reaction-axis and protein-axis F0-dependent Charbonnier conductances share one outgoing normalization at each product vertex, eliminating axis-order splitting",
