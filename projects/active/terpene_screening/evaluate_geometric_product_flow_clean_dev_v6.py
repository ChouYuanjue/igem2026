...
"""Clean-dev v6 evaluator with direct empirical-measure forcing on the same intrinsic product manifold.
...
OUT = ROOT / "results/geometric_product_flow_clean_dev_v6"
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        # The empirical positive measure is already a measure on the product
        # manifold. Do not pre-diffuse it: the Euler-Lagrange operator of the
        # variational field is itself the geometric propagation mechanism.
        # This avoids applying the same product geometry once to the source and
        # again to the field.
        field_source_measure = empirical_measure
        ...
        flowed, info = anisotropic_bregman_product_field(
            ...
            protein_graphs,
            strength=1.0,
            positive_measure=field_source_measure,
        ...
        diagnostics.append(
            {
                ...
                "target_source_positive_mass": float(field_source_measure[0].sum()),
                "target_source_positive_support": int(np.count_nonzero(field_source_measure[0])),
    ...
    summary = {
        "method": "direct_measure_intrinsic_product_geometry_charbonnier_bregman_field_v6",
        "fold": fold,
        ...
        "positive_measure_transport": "none: fold-train positive relation is restricted to the local chart, symmetrically degree-normalized as A_re/sqrt(d_r d_e), globally normalized to one joint probability measure, and enters the variational energy directly as the empirical source; propagation is performed only by the Euler-Lagrange operator",
