...
DEFAULT_GLOBAL = ROOT / 'data/terpene_embeddings/esmc600m_global_combined'
DEFAULT_STRUCTURAL = ROOT / 'data/terpene_structural_observations_v1'
DEFAULT_OUTPUT = ROOT / 'data/terpene_multiresolution_protein_geometry_v1'
...
def _aligned_global_embeddings(proteins: pd.DataFrame, directory: Path) -> tuple[np.ndarray,np.ndarray,dict[str,object]]:
    ...
    aliases=_alias_map(proteins)
    entries['protein_id']=entries.Entry.map(aliases)
    entries=entries[entries.protein_id.notna()].copy()
    by_pid={pid:g for pid,g in entries.groupby('protein_id',sort=False)}
    aligned=np.zeros((len(proteins),matrix.shape[1]),dtype=np.float32)
    available=np.zeros(len(proteins),dtype=bool)
    duplicate_max_disagreement=0.0
    for i,pid in enumerate(proteins.protein_id.astype(str)):
        g=by_pid.get(pid)
        if g is None or len(g)==0:
            continue
        rows=g.row.to_numpy(dtype=int)
        x=np.asarray(matrix[rows],dtype=np.float32)
        if len(x)>1:
            xn=x/np.maximum(np.linalg.norm(x,axis=1,keepdims=True),1e-12)
            duplicate_max_disagreement=max(
                duplicate_max_disagreement,
                float(1.0-np.min(xn@xn.T)),
            )
        aligned[i]=x.mean(axis=0)
        available[i]=True
    return aligned,available,{
        'source_rows':int(len(entries)),
        'available_proteins':int(available.sum()),
        'duplicate_alias_max_cosine_disagreement':float(duplicate_max_disagreement),
    }
...
def main() -> None:
    ...
    masks=[global_available]
    names=['global_esmc_chordal']
    structural_info={}
    ...
    manifest={
        'version':'terpene-multiresolution-protein-geometry-v1',
