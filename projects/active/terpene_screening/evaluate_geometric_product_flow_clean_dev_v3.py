...
"""Clean-dev v3 evaluator with an information-geometric adaptive reaction metric tensor.
...
from projects.active.terpene_screening.multiscale_geometry import (
    ...
    information_product_neighbours,
)

OUT = ROOT / "results/geometric_product_flow_clean_dev_v3"
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        selected_train, reaction_chart_info = information_product_neighbours(
        ...
        reaction_affinity, reaction_kernel_info = information_product_affinity(
        ...
        diagnostics.append(
            {
                ...
                "reaction_product_view_information": json.dumps(reaction_chart_info.get("view_information", [])),
                "reaction_graph_mean_view_information": json.dumps(reaction_kernel_info.get("mean_view_information", [])),
    ...
    summary = {
        "method": "information_geometric_reaction_tensor_charbonnier_bregman_field_v3",
        "fold": fold,
        ...
        "reaction_geometry": "locally conformal product metric on DRFP global, RDKitPlus whole-reaction, and atom-mapped reaction-center Jaccard coordinates with spatially varying entropy-deficit tensor coefficients",
        "reaction_view_dimensions": {"drfp_global": drfp_dim, "rdkitplus_whole": rdkit_dim, "mapped_center": center_dim},
        "reaction_view_policy": "each coordinate coefficient is normalized KL(local self-tuning kernel || uniform); flat coordinates fade continuously, sharp coordinates gain precision; no labels, thresholds, or learned inter-view weights; categorical one-hot block excluded to avoid hard partitions",
