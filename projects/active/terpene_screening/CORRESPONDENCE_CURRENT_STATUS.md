...
The frozen broad-domain mainline remains product-flow v8 under the unchanged 1903-query / 185,918-candidate protocol. Do not reopen the spent external-retention set. Do not retroactively claim that v8 and the MARTS correspondence defect are the same numerical operator; they currently share the scientific product-manifold object, not an identical solver.
...
Runtime authority:
`scripts/catalyst_finder/correspondence_geometry_service.py`.
...
runtime Catalyst Finder source, relevant geometric query-extension code and
