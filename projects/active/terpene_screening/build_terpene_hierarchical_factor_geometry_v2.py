...
PG=ROOT/'data/terpene_multiresolution_protein_geometry_v4'; RG=ROOT/'data/terpene_multiresolution_reaction_geometry_v2'; MOT=ROOT/'data/terpene_family_aware_motif_coordinates_v1'; OUT=ROOT/'data/terpene_hierarchical_factor_geometry_v2'
def sha(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
mapping=pd.read_csv(ROOT/'data/terpene_tps_active_site_xattn_v1/reaction_mapping_audit.csv',dtype=str).fillna('');
if mapping.reaction_id.astype(str).tolist()!=pd.read_csv(RG/'reaction_ids.csv',dtype=str).sort_values('row',key=lambda x: pd.to_numeric(x)).reaction_id.astype(str).tolist(): raise RuntimeError('mapping confidence order mismatch')
q=pd.to_numeric(mapping.mapping_confidence,errors='raise').to_numpy(dtype=np.float64)
rg,rinfo=hierarchical_pullback_refinement_affinity(rbase,ra,[rw,rj],[rwa,rja],[q,q],fallback_distance=rw,fallback_available=rwa)
...
manifest={'version':'terpene-hierarchical-factor-geometry-v2','definition':'global sequence / global reaction-difference coordinates define the base atlas topology; jointly observed structure, pocket, catalytic-motif, and reaction-center coordinates contribute normalized pullback energy only on existing base edges; missing refinements are neutral; one reaction lacking DRFP uses changed-atom Wasserstein only to instantiate its fallback local atlas','protein_refinements':pnames,'reaction_refinements':['center_transition_wasserstein','center_token_jaccard'],'reaction_observation_reliability':'pair reliability sqrt(q_i q_j) from current RXNMapper mapping_confidence; no threshold or learned transform','protein_info':pinfo,'reaction_info':rinfo,'labels_used':False,'input_sha256':{'protein_manifest':sha(PG/'manifest.json'),'reaction_manifest':sha(RG/'manifest.json'),'motif_manifest':sha(MOT/'manifest.json'),'reaction_mapping_audit':sha(ROOT/'data/terpene_tps_active_site_xattn_v1/reaction_mapping_audit.csv')}}
