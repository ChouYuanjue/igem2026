...
The second-round conformal and cycle-grid implementation report is
`docs/terpene_second_round_conformal_cycle_20260805_zh.md`. It records the
query-disjoint calibration protocol, route-bound retrieval-set sizes, API/CLI
contract and the negative cycle-reranking confirmation result.
...
few-shot routes, taxonomy-restricted R2E candidate universes and manual
model/routing overrides are not assigned an unrestricted external double-cold
reliability score.

## R2E enzyme taxonomy scopes

Reaction-to-enzyme retrieval can optionally restrict the **enzyme candidate
universe before scoring**. The default `all` mode is the existing 2,085-protein
production universe. `eukaryote` retains 1,340 locally classified eukaryotic
proteins; `prokaryote` retains 180 bacterial, archaeal and cyanobacterial
proteins. Six viral records are treated as `other`, and 559 unresolved records
are excluded from both restricted modes rather than guessed into a biological
domain.

```bash
# Keep the normal unrestricted workflow.
.venv/bin/python projects/active/terpene_screening/rank_open_world.py \
  rank-enzymes --reaction-id RHEA:54512 --top-k 10 \
  --enzyme-taxonomy-scope all

# Score only eukaryotic enzyme candidates.
.venv/bin/python projects/active/terpene_screening/rank_open_world.py \
  rank-enzymes --reaction-id RHEA:54512 --top-k 10 \
  --enzyme-taxonomy-scope eukaryote

# Score only prokaryotic enzyme candidates.
.venv/bin/python projects/active/terpene_screening/rank_open_world.py \
  rank-enzymes --reaction-id RHEA:54512 --top-k 10 \
  --enzyme-taxonomy-scope prokaryote
```

The local classification contract is generated by
`scripts/prepare_terpene_taxonomy_scope.py` into
`data/terpene_taxonomy_scope/protein_taxonomy_scope.csv`. Direct MARTS kingdom
labels are preferred; a conservative same-genus inference from the existing
local UniProt snapshot is used only when the genus has a unique MARTS kingdom.
Every row records taxonomy source and confidence. Restricted few-shot R2E
requires all supplied enzyme seeds to belong to the same selected scope.

Restricted route IDs are explicit, for example
`r2e-external-top10-v1+eukaryote-only` or
`r2e-current-top10-v1+fewshot+prokaryote-only`. Candidate-universe hashes and
sizes are recomputed after filtering. Because this changes the population that
is ranked, the unrestricted empirical-reliability and conformal calibrations
are intentionally not reused. E2R has no enzyme candidate universe and exposes
no taxonomy-scope control.

## Conformal retrieval sets

External zero-shot routes also expose route-bound, rank-based split-conformal
retrieval sets. The default is annotation-only at `alpha=0.10`; it reports the
prefix length required for a 90% marginal target of including at least one
known positive under the locked query-disjoint double-cold protocol and its
exchangeability assumptions. This is a retrieval coverage diagnostic, not an
activity probability.

```bash
# Keep the requested Top-10, but report the calibrated set size and truncation.
.venv/bin/python projects/active/terpene_screening/rank_open_world.py \
  rank-enzymes --reaction-smiles 'CCO>>CC=O' --query-id EXTERNAL_RXN \
  --top-k 10 --conformal-mode annotate --conformal-alpha 0.10

# Return the complete calibrated prefix while preserving the locked Top-10 route.
.venv/bin/python projects/active/terpene_screening/rank_open_world.py \
  rank-enzymes --reaction-smiles 'CCO>>CC=O' --query-id EXTERNAL_RXN \
  --top-k 10 --conformal-mode expand --conformal-alpha 0.10
```

Every conformal calibrator is bound to the route ID, model bundle and exact
candidate-universe hash. Current-library queries are marked
`not_applicable_current_entity`; few-shot, masked, taxonomy-restricted and
manual-override routes do not inherit the default external guarantee.
Applicability-aware Mondrian groups
are enabled only when their query-disjoint holdout coverage and sample-size
gates pass; otherwise the runtime falls back to the global calibrator.

The current 90% sets are intentionally large: the global R2E set is roughly
1,476–1,509 of 2,085 proteins and the global E2R set is 306–464 of 753
reactions, depending on the locked objective route. The system exposes these
numbers rather than presenting an unjustified small panel. Full methods and
validation tables are in
...
R2E batch discovery can use the same taxonomy restriction. Restricted batches
must use a distinct output directory; the program refuses to overwrite the
canonical unrestricted `results/terpene_registry_batch` output:

```bash
.venv/bin/python projects/active/terpene_screening/rank_registry_batch.py \
  --direction reaction_to_enzyme --objectives 3,10,20 \
  --r2e-enzyme-taxonomy-scope eukaryote \
  --output-dir results/terpene_registry_batch_eukaryote
```
