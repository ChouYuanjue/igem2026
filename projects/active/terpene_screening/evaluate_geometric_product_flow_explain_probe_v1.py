...
"""Explanation-only v8 probe with an order-symmetric two-factor mollifier on one reaction-protein product manifold.
...
OUT = ROOT / "results/geometric_product_flow_explain_probe_v1"
        ...
        diagnostics.append(
            {
                ...
                "top10_predicted_proteins": json.dumps([ids[int(x)] for x in flow_order[:10]]),
                "top10_predicted_rows": json.dumps([int(x) for x in flow_order[:10]]),
            }
        )
