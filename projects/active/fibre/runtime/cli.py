...
from projects.active.fibre.runtime.context import (  # noqa: E402
    protein_seed_similarity_expert,
    reaction_seed_similarity_expert,
    wrap_context_scores,
)
from projects.active.fibre.core.cache import (  # noqa: E402
    DEFAULT_FEATURE_CACHE,
    FeatureCache,
    stable_digest,
)
from projects.active.fibre.core.input_audit import (  # noqa: E402
    ProteinInputAudit,
    ReactionInputAudit,
...
from projects.active.fibre.core.provenance import (  # noqa: E402
...
from projects.active.fibre.runtime.reaction_similarity import (  # noqa: E402
    reaction_features as zero_shot_reaction_features,
    reaction_similarity as zero_shot_reaction_similarity,
)
from projects.active.fibre.runtime.protein_embeddings import mean_embedding  # noqa: E402
from projects.active.fibre.runtime.pair_model import (  # noqa: E402
    DualKernelAssets,
...
from projects.active.fibre.runtime.reaction_encoding import (  # noqa: E402
    canonical_or_raw_reaction,
    precursor_class_from_reaction,
    product_skeleton_class,
)
from projects.active.fibre.runtime.base_model import (  # noqa: E402
    ModelConfig,
    ProjectionTower,
    TerpeneDualTower,
...
def load_e2r_anchored_lambdamart_v3_runtime_cached(device: str):
    ...
    from projects.active.fibre.runtime.reaction_ranker import AnchoredE2RRuntime
...
def load_bime_e2r_v4_runtime_cached(
    ...
    from projects.active.fibre.runtime.enzyme_to_reaction import BiMEE2RRuntime
...
def rank_reactions_bime_e2r_v4(
    ...
    if seed_ids and bool(seed_context_settings.get("enabled", False)):
        from projects.active.fibre.runtime.positive_context import fuse_r2e_seed_context
...
def _encode_runtime_rdkitplus_extension(reaction_smiles: str) -> np.ndarray:
    ...
    key = "runtime_query"
    with tempfile.TemporaryDirectory(prefix="fibre_rdkitplus_runtime_") as temp_dir:
...
def _runtime_rxnmapper():
    ...
    from projects.active.fibre.runtime.reaction_mapping import load_mapper
...
def _encode_runtime_reaction_center_extension(
    ...
    from projects.active.fibre.runtime.reaction_features import (
...
def encode_exact_horizyn_reactions(
    ...
    from projects.active.fibre.runtime.reaction_adapter import (
...
def rank_enzymes(args: argparse.Namespace) -> pd.DataFrame:
    ...
    if lambdarank_spec is not None:
        from projects.active.fibre.runtime.enzyme_ranker import fuse_r2e_scores
        ...
        if bool(structure_expert_spec.get("enabled", False)):
            from projects.active.fibre.runtime.reaction_to_enzyme import fuse_bime_r2e_scores
