...
from projects.active.fibre.runtime import reaction_model as v3
...
def _clip_assets(candidate_ids: list[str], query_ids: list[str]) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    Candidate vectors are dense in the canonical reaction order; unsupported rows are zero and
