...
explicit MIT license. project-specific retrieval losses and Fisher data
sampling remain outside this vendored module.

`fusionbench_regmean.py` similarly extracts the generic Linear RegMean merge core from `fusion_bench/method/regmean/regmean.py` at the same upstream commit. The project uses `torch.linalg.pinv` rather than `torch.inverse` for low-rank activation Gram stability.
