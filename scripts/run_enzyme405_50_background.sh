...
launch_compare_baseline() {
  ...
  nohup "${PYTHON}" explorations/pocket/runners/run_compare_baselines.py \
...
launch_derived_baseline() {
  ...
  local cmd=(
    "${PYTHON}" explorations/pocket/analysis/derive_baseline_from_topk.py
...
write_blocked_fpocket_summary() {
  ...
  local config_path="explorations/pocket/configs/generated_best_available/${run_id}.yaml"
  mkdir -p "${run_dir}"
...
if result_complete "enzyme405_50_official_precomputed_pocket" | grep -q '^1$'; then
  ...
  launch_compare_baseline \
    ...
    "explorations/pocket/configs/generated_best_available/enzyme405_50_official_precomputed_pocket.yaml"
fi
...
if result_complete "enzyme405_50_p2rank_topk_softmax_pool" | grep -q '^1$'; then
  ...
  launch_compare_baseline \
    ...
    "explorations/pocket/configs/generated_best_available/enzyme405_50_p2rank_topk_softmax_pool.yaml"
fi
...
if command -v fpocket >/dev/null 2>&1; then
  ...
  do
    ...
    launch_compare_baseline \
      "${run_id}" \
      "explorations/pocket/configs/generated_best_available/${run_id}.yaml"
  done
else
  for run_id in \
    enzyme405_50_fpocket_top1 \
