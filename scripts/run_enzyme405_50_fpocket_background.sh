...
launch_compare_baseline() {
  ...
  nohup setsid "${PYTHON}" explorations/pocket/runners/run_compare_baselines.py \
...
launch_compare_baseline_when_source_ready() {
  ...
  nohup setsid env \
    ...
    bash -c '
      ...
      exec "${PYTHON}" explorations/pocket/runners/run_compare_baselines.py --experiment_config "${CONFIG_PATH}" --resume
...
if result_complete "enzyme405_50_fpocket_topk_rank_weighted" | grep -q '^1$'; then
  ...
  launch_compare_baseline \
    ...
    "explorations/pocket/configs/generated_best_available/enzyme405_50_fpocket_topk_rank_weighted.yaml"
fi
...
if result_complete "enzyme405_50_fpocket_top1" | grep -q '^1$'; then
  ...
  launch_compare_baseline_when_source_ready \
    ...
    "explorations/pocket/configs/generated_best_available/enzyme405_50_fpocket_top1.yaml" \
    "enzyme405_50_fpocket_topk_rank_weighted"
fi
...
if result_complete "enzyme405_50_p2rank_fpocket_union_max" | grep -q '^1$'; then
  ...
  launch_compare_baseline \
    ...
    "explorations/pocket/configs/generated_best_available/enzyme405_50_p2rank_fpocket_union_max.yaml"
fi
...
if result_complete "enzyme405_50_p2rank_fpocket_union_source_weighted" | grep -q '^1$'; then
  ...
  launch_compare_baseline \
    ...
    "explorations/pocket/configs/generated_best_available/enzyme405_50_p2rank_fpocket_union_source_weighted.yaml"
fi
