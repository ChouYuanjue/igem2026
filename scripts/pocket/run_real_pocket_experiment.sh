...
GENERATED_DIR="projects/active/pocket_robustness/configs/generated"
...
run_step "inspect_enzymecage_assets" bash scripts/pocket/inspect_enzymecage_assets.sh
run_step "inspect_enzymecage_configs" bash scripts/pocket/inspect_enzymecage_configs.sh
run_step "inspect_enzymecage" bash scripts/pocket/inspect_enzymecage.sh
...
run_step "compare_all_baselines" \
  "${PYTHON}" projects/active/pocket_robustness/analysis/compare_all_baselines.py \
...
if [[ -n "${P2RANK_MANIFEST}" && -n "${FPOCKET_MANIFEST}" ]]; then
  run_step "compare_pocket_sources" \
    "${PYTHON}" projects/active/pocket_robustness/analysis/compare_pocket_sources.py \
...
run_step "summarize_experiment_status" \
  "${PYTHON}" projects/active/pocket_robustness/analysis/summarize_experiment_status.py \
    --results_root results/pocket \
    --baseline_matrix "${MATRIX}" \
