...
TOOL_CATALOG: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "args": {"protein_scope_ref": "family/class ref returned by resolve_protein_scope or broaden_scope", "limit": "1..30"},
    },
    {
    ...
    },
    {
        ...
        "purpose": "Build only the research modules explicitly requested for one verified concrete protein or reaction. Modules are composable: annotations, structures, literature, recorded_relations, model, next_steps. A relation-only question belongs in lookup_relations; use this tool when the user asks for one or more additional research modules or a broad research overview.",
...
class ScientificToolRegistry:
        ...
        if structured.reaction is not None:
            ...
            return ToolResult(
                ...
                summary=(
                    ...
                    "For a recorded-evidence question, use lookup_relations with this reaction_ref; for model discovery, reuse this reaction_ref in candidate_search."
                ),
                payload={
                    "reaction_ref": ref,
                    "recommended_id": item.query_id,
    ...
    def _tool_resolve_protein_scope(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        ...
        if args.scope_hint == "family_or_class":
            ...
            # The strict functional-class scope is defined by the canonical
            # protein terms from parse_protein, not by stochastic synonym
            # expansion. This keeps equivalent Chinese/English requests on the
            # same auditable member set. Expansion contributes only optional
            # aliases/parent terms and never silently widens the strict scope.
            canonical_terms = list(normalized.get("protein_terms") or [])
            strict_terms = canonical_terms or list(expanded.get("strict_terms") or [])
            broader_terms = list(expanded.get("broader_terms") or [])
            strict_aliases = [
                value for value in (expanded.get("strict_terms") or [])
                if str(value).casefold() not in {str(term).casefold() for term in strict_terms}
            ]
            enzyme_spec = {
                "raw_text": text,
    ...
    def _tool_lookup_relations(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        if args.reaction_ref:
            return self._lookup_relations_by_reaction(args, ctx)
        ...
        if str(scope.get("kind") or "") == "specific_protein":
            return self._lookup_relations_by_protein(args, ctx)
        return self._lookup_relations_by_scope(args, ctx)

    def _lookup_relations_by_reaction(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
    ...
    def _lookup_relations_by_protein(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        scope = ctx.protein_refs.get(args.protein_scope_ref)
        if scope is None:
            raise AppError("unknown_protein_scope_ref", "The protein_scope_ref is not available in this harness run.", 422)
        if str(scope.get("kind") or "") != "specific_protein":
    ...
    def _tool_list_scope_members(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        scope = ctx.protein_refs.get(args.protein_scope_ref)
        if scope is None:
            raise AppError("unknown_protein_scope_ref", "The protein_scope_ref is not available in this harness run.", 422)
        kind = str(scope.get("kind") or "")
    ...
    def _tool_inspect_entity(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        zh = str(ctx.ui_language or "").lower().startswith("zh")
        entity_kind = ""
        entity: dict[str, Any]
        note = ""
    ...
    def _tool_compare_entities(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        zh = str(ctx.ui_language or "").lower().startswith("zh")
        entities: list[dict[str, Any]] = []
        kinds: list[str] = []
    ...
    def _tool_research_workspace(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
    ...
    def _lookup_relations_by_scope(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        scope = ctx.protein_refs.get(args.protein_scope_ref)
        if scope is None:
            raise AppError("unknown_protein_scope_ref", "The protein_scope_ref is not available in this harness run.", 422)
        kind = str(scope.get("kind") or "")
        ...
        if kind == "functional_class" and count == 0 and not bool((scope.get("enzyme_spec") or {}).get("scope_broadened")):
            ...
            if broader:
                return ToolResult(
                    tool="lookup_relations",
                    status="error",
                    summary="The strict functional-class scope produced no recorded reaction evidence; the scope can be explicitly broadened with broaden_scope.",
    ...
    def _tool_broaden_scope(self, args: Any, ctx: HarnessRunContext) -> ToolResult:
        scope = ctx.protein_refs.get(args.protein_scope_ref)
        if scope is None:
            raise AppError("unknown_protein_scope_ref", "The protein_scope_ref is not available in this harness run.", 422)
                ...
                if reaction_ref:
                    ...
                    if not reaction_text:
                        ...
                        return ToolResult(
                            ...
                            status="error",
                            summary=(
                                "This request contains a protein sequence but no reaction target. If the user's goal is to find reactions the protein may catalyze, retry candidate_search with direction=enzyme_to_reaction; do not treat the amino-acid sequence as a reaction description."
                                if sequence_only
