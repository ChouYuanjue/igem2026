...
RUNTIME_DIR="${STARASE_NAVIGATOR_RUNTIME_DIR:-${ROOT_DIR}/results/starase_navigator_runtime}"
TMP_DIR="${STARASE_NAVIGATOR_TMP_DIR:-${RUNTIME_DIR}/tmp}"
PID_FILE="${RUNTIME_DIR}/server.pid"
LOG_FILE="${RUNTIME_DIR}/server.log"
ENV_FILE="${RUNTIME_DIR}/deepseek.env"
HOST="${STARASE_NAVIGATOR_HOST:-127.0.0.1}"
PORT="${STARASE_NAVIGATOR_PORT:-8791}"
PYTHON="${STARASE_NAVIGATOR_PYTHON:-${ROOT_DIR}/.venv/bin/python}"
SERVER="${ROOT_DIR}/scripts/starase_navigator/serve.py"
SYSTEMD_UNIT_NAME="starase-navigator.service"
SYSTEMD_UNIT_SOURCE="${ROOT_DIR}/scripts/starase_navigator/starase-navigator.service"
...
is_running() {
  ...
  [[ "${cmdline}" == *"scripts/starase_navigator/serve.py"* ]]
}
...
feedback_summary() {
  "${PYTHON}" "${ROOT_DIR}/scripts/starase_navigator/feedback_report.py" --limit "${1:-10}"
}

feedback_tail() {
  "${PYTHON}" "${ROOT_DIR}/scripts/starase_navigator/feedback_report.py" --raw --limit "${1:-20}"
}

feedback_json() {
  "${PYTHON}" "${ROOT_DIR}/scripts/starase_navigator/feedback_report.py" --json --limit "${1:-20}"
}
