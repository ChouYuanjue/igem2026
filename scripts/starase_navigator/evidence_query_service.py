...
from scripts.starase_navigator.formatting import probable_uniprot as _probable_uniprot
        ...
        *,
        ...
        return {
            ...
            "known_associations": {
                ...
                "integrated_database_count_source": "integrated_evidence_catalog",
                "items": items,
                "truncated": False,
            ...
            "route_view": {
                ...
                "nodes": [
                    ...
                    },
                    {
                        ...
                        "subtitle": "整合证据库" if zh else "Integrated evidence catalog",
                        "kind": "evidence",
                        "detail": note,
                        "metric": f"{len(ids)} 个已记录蛋白" if zh else f"{len(ids)} recorded proteins",
                    },
                    {
                        ...
                        "kind": "decision",
                        "detail": note,
                        "metric": f"{len(items)} 个匹配蛋白" if zh else f"{len(items)} matched proteins",
                    },
                    {
                        "id": "evidence-output",
                        "title": "返回数据库记录" if zh else "Return database records",
                        "subtitle": "仅数据库证据" if zh else "Evidence only",
                        "kind": "output",
                        "detail": note,
                        "metric": f"{len(items)} 个蛋白" if zh else f"{len(items)} proteins",
                    },
                ],
                "edges": [
            ...
            "route_view": {
                ...
                "nodes": [
                    ...
                    },
                    {
                        ...
                        "kind": "evidence",
                        "detail": note,
                        "metric": f"{len(items)} 条已记录反应" if zh else f"{len(items)} recorded reactions",
                    },
                ],
