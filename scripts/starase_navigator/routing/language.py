    ...
    def next_harness_action(
        ...
        *,
        ...
        system_prompt = (
            ...
            "role=model_candidate/source=executed_model_candidate are server-executed model hypotheses from an earlier result: they may be "
            "inspected, compared, searched for supporting literature, or otherwise investigated, but their association to the original "
            "target is NOT verified evidence. Do not restart the original candidate search merely because the user asks about or compares "
            "candidate objects that already have reusable handles. "
            "Workspace route and route_step handles are server-executed route objects, not prose summaries. For follow-ups such as "
            ...
            "creating a separate route-only evidence workflow. Derived route handles expose parent_route_id, root_route_id, generation, and patch step range; use these object relations "
            "to resolve phrases such as 'the original route', 'the first patched route', 'compare it with its parent', or a later edit to an already patched route. Do not reconstruct lineage "
            "from prior assistant prose when typed lineage is available. When the user asks to replace, reroute, or modify only one step or one contiguous part of an existing route, "
            ...
            "\n\n"
            "A tool error or missing direct capability is an observation, not a command to give up. Revise the plan and compose "
            "available tools to make useful progress: inspect existing workspace objects, search/resolve relevant literature or records, "
            ...
            "Database and literature searches are scoped, not exhaustive: when a search returns no direct match, say that no direct evidence was found in the performed search/current sources, not that evidence is globally zero or nonexistent. "
            "Treat numeric evidence counts the same way: null, absent, or not_computed is not zero. Counts from different named sources, snapshots, releases, or filtered scopes are separate observations and need not match. When they differ, report the values with their provenance/scope instead of silently substituting one for another or declaring an implementation error unless a verified observation establishes that. "
    ...
    def synthesize_grounded_answer(
        ...
        *,
        ...
        system_prompt = (
            ...
            "not proof that something does not exist globally. If latest_verified_result is a verification state with needs_user_input=true, do not pretend the requested scientific "
            "workflow has executed. Explain exactly which identity or choice remains ambiguous using only the structured candidates/options present in that result, say why the choice "
            "matters, and ask only for the minimum necessary selection. If the evidence is partial, give the most useful scoped answer and state the important limitation naturally. "
