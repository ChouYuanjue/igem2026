...
def configured_expert_roots() -> tuple[Path | None, Path | None]:
    ada_raw = os.environ.get("STARASE_NAVIGATOR_GENERAL_ADAMERGING_DIR", str(DEFAULT_ADAMERGING_EXPERT)).strip()
    full_raw = os.environ.get("STARASE_NAVIGATOR_GENERAL_FULL_DIR", str(DEFAULT_FULL_EXPERT)).strip()
