...
bash scripts/starase_navigator/manage.sh start
bash scripts/starase_navigator/manage.sh status
```

Runtime state and secrets live under ignored paths, primarily:

```text
results/starase_navigator_runtime/
```

Configure DeepSeek without placing the key in tracked source:

```bash
bash scripts/starase_navigator/manage.sh configure-key
```
...
```bash
.venv/bin/python scripts/maintenance/run_reproduction_tests.py --tier extended
```

Before deployment, also run:

```bash
python3 -m compileall -q scripts/starase_navigator
