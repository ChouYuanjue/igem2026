...
项目的模型主线最早就是为这个场景做的反应找酶检索。蛋白侧先用 ESM-C 表示通用序列与家族信息\cite{esmc2024}，随后加入更偏向酶功能语义的 EnzGFM\cite{enzgfm2026}；反应侧用 DRFP 和 RDKit 描述整体化学变化\cite{drfp2022,rdkit}，再用 RXNMapper 恢复原子对应关系\cite{rxnmapper2021}。后者让模型知道“哪些键真的发生了变化”，而不只是看到反应物和产物的整体指纹。

早期 Catalyst 专项原型让我们确认了一件更重要的事：只要把目标从“给候选分类”改成“在固定实验预算里排序”，模型就能直接对应实验决策。那一阶段的数值属于 BiME-Rank 形成之前的旧 Catalyst route；为了避免把前身模型成绩误写成当前系统能力，科研 release 不再把这些历史数字放进当前结果表。真正的 current BiME-v2 外部结果统一放在后面的同协议比较中。
...
专家准入统一遵循三条标准：真实输入可执行、能够持续给出候选级分数、在内部开发数据上证明互补价值；外部测试不参与选择。当前 release 不再把历史 Catalyst 阶段的内部确认数字包装成 BiME-Rank 的整体能力；零样本能力统一放到后面的同协议外部比较中，条件式能力则只使用当前 V2 scorecard 已冻结的 seed-context 结果。

\section{模型真正要面对的，是研究者手上已经有多少可靠信息}

同一个检索问题，在“完全没有已知阳性”和“实验室已经验证了一个能工作的候选”两种状态下并不是同一个任务。BiME-Rank 因此把零样本排序和已知阳性条件式重排分开：前者保持完整 frozen order，后者只在明确提供 seed 时启用 context ranker，并且把 seed 本身从最终答案中屏蔽。

\subsection{一个已验证阳性时，当前模型如何利用它}

当前 one-seed 条件式评测仍遵守严格时间外边界。R2E 在 39 个可评 query、90 个 trial 上，MRR 为 \textbf{0.5801}，Hit@10/20/50 分别为 \textbf{75.56\% / 78.89\% / 82.22\%}，最佳正例中位名次为 \textbf{2}；E2R 在 35 个可评 query、80 个 trial 上，MRR 为 \textbf{0.5257}，Hit@10/20/50 为 \textbf{83.75\% / 92.50\% / 95.00\%}，最佳正例中位名次为 \textbf{3}。这些数字只描述“已知一个阳性”的条件式检索，不与 zero-shot 指标混写。

\begin{table}[htbp]
\centering
\small
\caption{冻结 one-seed context ranker 的严格时间外条件式检索}
\begin{tabular}{lrrrr}
\toprule
\textbf{方向} & \textbf{MRR} & \textbf{Hit@10} & \textbf{Hit@20} & \textbf{Hit@50}\\
\midrule
R2E & \textbf{0.5801} & \textbf{75.56\%} & \textbf{78.89\%} & \textbf{82.22\%}\\
E2R & \textbf{0.5257} & \textbf{83.75\%} & \textbf{92.50\%} & \textbf{95.00\%}\\
\bottomrule
\end{tabular}
\end{table}

\subsection{多个阳性不需要另起一套模型}

context ranker 仍只用 one-seed 条件训练与冻结，随后在\textbf{同一个 hidden target、嵌套的 1/2/3/5 seed 集合}上直接测试，没有为多 seed 重新调参。R2E 从 1 个 seed 增长到 5 个 seed 时，MRR 从 0.0355 增至 \textbf{0.0589}，Hit@50 从 27.95\% 增至 \textbf{42.94\%}，hidden target 的中位名次从 934 提升到 \textbf{75}；E2R 的 MRR 从 0.0956 增至 \textbf{0.1968}，Hit@10 从 22.92\% 增至 \textbf{43.75\%}。这里的目的不是声称 seed 越多每个指标都单调上升，而是验证同一冻结规则能够接收多阳性集合而无需重新训练。
...
\subsection{没有通过准入的信号，不进入当前模型}

资产中仍保留 homology-context、reciprocal-consistency 和 EnzymeCAGE Top-20 expert 的实验，是为了让负结果也能复核，而不是表示这些分支仍在 production。当前 V2 admission contract 中，CLIPZyme 结构信号与 seed-context 已准入；homology-context 在冻结外部 retention 上造成主要排序指标退化，reciprocal-consistency 的内部增益没有迁移到冻结外部确认，EnzymeCAGE Top-20 expert 也没有通过同容量内部 OOF。因此三者都不属于当前 BiME-Rank 排序专家。这个边界比继续展示旧阶段“某个切片曾经很高”的数字更重要。

\subsection{分层执行已经用于真实实验候选库}

成本感知不是一个额外的排名专家，而是当前已准入专家之上的执行策略。一次 17 个目标反应的实验规划面对 \textbf{131,532 个本地蛋白候选}：第一阶段按每反应 Top-50 冻结廉价信号，跨反应去重后只需为 \textbf{530 个蛋白，占原库 0.40\%} 补算昂贵表示；现场 exact EnzGFM-650M 计算耗时 \textbf{35.82 s}。最终实验表覆盖 17 个唯一反应、16 个唯一 PRIMARY 构建（含 backup 时共 27 个）。这组数字只说明 on-demand feature materialization 如何把当前排序系统落到实验预算上，不作为新的外部 benchmark。
...
EnzymeCAGE & \textbf{强结构外部基线；已测试但未准入当前专家池} & 当前 BiME-v2 在 Enzyme-405 上完成官方检查点同协议比较；另在 462-query 固定 Top-20 内部 OOF 中作为候选结构专家测试，但 MRR/MAP/NDCG/Hit@10 未改善，因此当前不激活。\\
\bottomrule
\end{tabularx}
\end{table}
...
本地复现外部对照 & EnzymeCAGE；CLIPZyme & 两者均使用官方实现/检查点完成当前 BiME-v2 的同协议比较\\
\bottomrule
\end{longtable}
