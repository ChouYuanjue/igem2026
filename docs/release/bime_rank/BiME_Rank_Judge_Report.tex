...
成本感知不是一个额外的排名专家，而是当前已准入专家之上的执行策略。一次 17 个目标反应的实验规划面对 \textbf{131,532 个本地蛋白候选}：第一阶段按每反应 Top-50 冻结廉价信号，跨反应去重后只需为 \textbf{530 个蛋白，占原库 0.40\%} 补算昂贵表示；现场 exact EnzGFM-650M 计算耗时 \textbf{37.33 s}。最终实验表覆盖 17 个唯一反应、16 个唯一 PRIMARY 构建（含 backup 时共 27 个）。这组数字只说明 on-demand feature materialization 如何把当前排序系统落到实验预算上，不作为新的外部 benchmark。
