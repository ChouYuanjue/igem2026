...
## 四、任务形式化：R2E、E2R、Zero-shot 与 Few-shot

令反应集合为 \(\mathcal{R}\)，蛋白集合为 \(\mathcal{P}\)，已知关联为 \(A\subseteq\mathcal{R}\times\mathcal{P}\)。系统学习或构造一个打分函数 \(s(r,p)\)。
...
|---|---:|---:|---:|---:|---:|---:|---:|
| Exact 蛋白未见；同簇同源物允许可见 | R2E | 954 | 39.1% | 44.8% | 51.2% | 56.9% | 0.350 |
| Exact 蛋白未见；同簇同源物允许可见 | E2R | 917 | 60.4% | 64.7% | 72.4% | 77.5% | 0.552 |
| Exact 反应未见；同簇相似反应允许可见 | R2E | 513 | 25.7% | 29.6% | 38.0% | 45.2% | 0.248 |
| Exact 反应未见；同簇相似反应允许可见 | E2R | 1413 | 23.0% | 29.7% | 37.9% | 48.1% | 0.210 |
...
|---|---:|---:|---:|---:|---:|---:|
| Exact 蛋白未见；同簇同源物允许可见 | R2E | 有同簇训练邻居 | 722 | 49.9% | 62.6% | 68.1% |
| Exact 蛋白未见；同簇同源物允许可见 | R2E | 无同簇训练邻居 | 232 | 5.6% | 15.5% | 22.0% |
| Exact 蛋白未见；同簇同源物允许可见 | E2R | 有同簇训练邻居 | 706 | 71.8% | 82.6% | 86.7% |
| Exact 蛋白未见；同簇同源物允许可见 | E2R | 无同簇训练邻居 | 211 | 22.3% | 38.4% | 46.9% |
| Exact 反应未见；同簇相似反应允许可见 | R2E | 有同簇训练邻居 | 298 | 30.5% | 44.6% | 52.3% |
| Exact 反应未见；同簇相似反应允许可见 | R2E | 无同簇训练邻居 | 215 | 19.1% | 28.8% | 35.3% |
| Exact 反应未见；同簇相似反应允许可见 | E2R | 有同簇训练邻居 | 951 | 28.8% | 44.6% | 55.5% |
| Exact 反应未见；同簇相似反应允许可见 | E2R | 无同簇训练邻居 | 462 | 11.0% | 24.2% | 32.9% |
...
| UniProt raw | 46,064 | 五个 TPS 相关 Pfam 查询 |
| UniProt novel exact-sequence unique | 43,812 | 移除 current/MARTS 重复后 |
| UniProt 50% identity clusters | 6,494 | MMseqs 代表 |
| 启用的 named UniProt | 5,672 | A–D 证据层 |
| 暂不启用 domain-only | 822 | E 层 |
...
### 9.1 Legacy exact-reaction protocol
...
### 9.2 Common current-only double-cold

| 方法 | Hit@5 | Hit@10 | Hit@20 |
|---|---:|---:|---:|
| 旧 fold-local RF rescue | 0.0% | 0.6% | 1.5% |
| 新 controlled dual tower | 4.2% | 9.0% | 16.8% |
| 绝对差值 | +4.20 pp | +8.40 pp | +15.27 pp |

在真正隔离蛋白簇与反应簇后，旧 fold-local RF rescue 基本失效，而新双塔仍维持可用检索能力。Top-10 的配对 bootstrap 区间为 +6.46 pp 到 +10.33 pp；Top-20 为 +12.24 pp 到 +18.40 pp。这比只比较两个均值更有说服力。
...
| E2R | Top-20 | 0.70 freeze-route + 0.30 dual-kernel RRF, c=60 | 39.2% |
...
| 阶段 | Query-cells | 融合 Hit@20 | 原生产 Hit@20 | 差值/诊断 |
|---|---:|---:|---:|---:|
| 开发 9 cells（选参） | — | 49.6% | — | MRR 0.087 |
| 原冻结 16 cells | 153 | 34.0% | 28.8% | +5.23 pp; CI [-1.31 pp, +11.76 pp] |
| 独立锁定 fold seed 20260726 | 279 | 43.4% | 34.8% | +8.60 pp; CI [+5.02 pp, +12.54 pp] |

独立确认中新增命中 27 个、丢失 3 个，净增加 24 个 query-cell；MRR 从 0.0764 提升到 0.0874。置信区间下界大于 0，说明提升不只是单个切分上的偶然方向。
...
|---|---:|---:|---:|---:|---:|---:|
| enzyme_to_reaction_top10 | 是 | 0.711 | [0.632, 0.787] | 0.545 | 25.4% | 0.203 |
| enzyme_to_reaction_top3 | 是 | 0.874 | [0.802, 0.936] | 0.344 | 7.8% | 0.144 |
| reaction_to_enzyme_top10 | 是 | 0.626 | [0.519, 0.731] | 0.185 | 13.5% | 0.234 |
| reaction_to_enzyme_top20 | 是 | 0.610 | [0.514, 0.704] | 0.338 | 19.0% | 0.243 |
| reaction_to_enzyme_top3 | 否 | 0.435 | [0.270, 0.622] | 0.049 | 4.6% | 0.242 |
| enzyme_to_reaction_top20 | 是 | 0.695 | [0.627, 0.758] | 0.612 | 39.2% | 0.219 |

E2R Top-20 新校准器 ROC-AUC 为 0.695，bootstrap 95% CI 为 [0.627, 0.758]。最高可靠性四分位的 Hit@20 为 65.7%，总体为 39.2%。
...
| 有效 Top-1 候选数 | 127.8 → 139.1 |
| 不同 Top-1 候选数 | 202 → 217 |
| 外部反应成为 Top-1 的比例 | 8.9% → 11.0% |
...
## 二十、UniProt 扩展：为什么不能把 5,672 条序列直接混入主排名

| 预算 | canonical + UniProt | 受控保留率 | 自由合并保留率 |
|---|---:|---:|---:|
| Top-3 | 3 + 0 | 100.0% | 54.5% |
| Top-10 | 9 + 1 | 93.3% | 50.0% |
| Top-20 | 18 + 2 | 97.7% | 58.1% |
...
## 二十一、Reaction-specific Pfam architecture contract

240 个注册反应中，208 个支持五-Pfam rescue，32 个保持 canonical-only。支持状态包括 195 个 reference-supported 和 13 个 multi-architecture 反应。
...
| 总氨基酸 | 184,501 |
| 总 coding nt（无 stop） | 553,503 |

canonical 四块板包含 288 个发现 assay、48 个阳性对照、24 个空载体阴性和 24 个底物/流程空白。canonical 去重构建为 248 条。

rescue 两块板覆盖 24 个反应、96 个候选 assignment、93 个不同 UniProt 候选；高置信序列风险为 0。证据层分布为 A=2、B=28、C=40、D=26。

### 22.1 MILP 板间平衡

目标是在每块板容量固定的条件下平衡萜类型、TPS class、底物、对照、候选长度、外部候选比例、证据层和 Pfam 架构。求解器状态为 optimal。

- canonical candidate median-length mean 板间 range：146.750 → 6.833 aa；
- canonical q90-length mean range：221.650 → 30.533 aa；
- rescue median-length mean range：100.125 → 4.875 aa；
- rescue q90-length mean range：107.483 → 0.750 aa；
...
- mean normalized slot entropy：0.201 → 0.974；
- maximum single-slot role share：100.0% → 33.3%；
...
| 项目 | 历史记录 | 当前正式值 | 说明 |
|---|---:|---:|---:|
| 六块板不同反应数 | 早期总结 29 | 当前正式 39 | 以 combined campaign summary 为准 |
| 主构建去重数 | 早期 348 | 当前正式 352 | 以 master constructs 为准 |
| rescue 平均长度 range | 早期 1.5 aa | 当前正式 4.875 aa | Pfam 精确平衡修复后 |
| E2R Top-20 | 旧生产 32.5%/34.0% | 当前严格校准 39.2%; 独立确认 43.4% | 双核 RRF 后 |
...
.venv/bin/python projects/active/terpene_screening/evaluate_open_world_uncertainty.py   --output-dir results/terpene_open_world_uncertainty_dual_kernel_candidate_v1   --device cuda
.venv/bin/python projects/active/terpene_screening/rank_registry_batch.py   --direction both --objectives 3,10,20 --device cuda   --output-dir results/terpene_registry_batch
...
## 三十三、第二轮 Conformal Retrieval Sets 与循环权重网格
