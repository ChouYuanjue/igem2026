...
V5 = ROOT / "results/geometric_product_flow_clean_dev_v8"
...
def main() -> None:
    ...
    if not v5[["fold", "query_id"]].equals(v5_baseline[["fold", "query_id"]]):
        raise RuntimeError("v8/v5 query keys differ")
    if not v5[["fold", "query_id"]].equals(bime[["fold", "query_id"]]):
        raise RuntimeError("v8/BiME query keys differ")
    ...
    result = {
        ...
        "delta_v8_vs_v5": {k: metric_map(v5)[k] - metric_map(v5_baseline)[k] for k in metric_map(v5)},
        "delta_v8_vs_v1": {k: metric_map(v5)[k] - metric_map(v1)[k] for k in metric_map(v5)},
        "delta_v5_vs_bime": {k: metric_map(v5)[k] - metric_map(bime)[k] for k in metric_map(v5)},
        "folds": folds,
        "convergence": convergence,
        "leakage_audit": leakage,
        "paired_bootstrap_v8_vs_v5": bootstrap(v5, v5_baseline),
        "paired_bootstrap_v5_vs_v1": bootstrap(v5, v1),
        "paired_bootstrap_v5_vs_bime": bootstrap(v5, bime),
        "provenance_sha256": {
            "evaluator": sha256(ROOT / "archive/terpene_screening/lineage/evaluate_geometric_product_flow_clean_dev_v8.py"),
    ...
    (V5 / "paired_bootstrap_vs_v5.json").write_text(json.dumps(result["paired_bootstrap_v8_vs_v5"], indent=2) + "\n")
