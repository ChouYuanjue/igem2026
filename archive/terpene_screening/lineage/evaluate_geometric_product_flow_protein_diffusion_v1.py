...
"""Clean-dev evaluator for intrinsic one-step diffusion geometry on the protein axis.
...
from projects.active.terpene_screening.geometry.multiscale import one_step_diffusion_affinity

OUT = ROOT / "results/geometric_product_flow_protein_diffusion_v1"
...
def evaluate_fold(
    ...
    # Reaction geometry is chemical, not learned association geometry.  The last
    # block is the deterministic atom-mapped reaction-centre representation
    # (before/after centred Morgan fingerprints + explicit transition tokens).
    ...
    reaction_center = np.asarray(reaction_features[:, -center_dim:], dtype=np.float32)
    center_available = np.any(reaction_center != 0, axis=1)
    valid_train_mask = center_available[train_rows_np]
    valid_train_rows_np = train_rows_np[valid_train_mask]
    valid_train_ids = [r for r, ok in zip(train_reactions, valid_train_mask) if ok]
    ...
    effective_reaction_neighbours = min(
        ...
        else int(math.ceil(math.sqrt(max(1, len(valid_train_ids))))),
    )
    center_t = torch.as_tensor(reaction_center, dtype=torch.float32, device=device)
    valid_train_rows_t = torch.as_tensor(valid_train_rows_np, dtype=torch.long, device=device)
    query_rows_np = np.asarray([rindex[q] for q in query_ids], dtype=np.int64)
    query_rows_t = torch.as_tensor(query_rows_np, dtype=torch.long, device=device)
    with torch.no_grad():
        qcenter = center_t[query_rows_t]
        tcenter = center_t[valid_train_rows_t]
        intersection = qcenter @ tcenter.T
        qmass = qcenter.sum(dim=1, keepdim=True)
        tmass = tcenter.sum(dim=1)[None, :]
        union = qmass + tmass - intersection
        reaction_center_similarity_to_train = torch.where(
            union > 0, intersection / union.clamp_min(1e-12), torch.zeros_like(union)
        ).cpu().numpy()
    ...
    for qi, q in enumerate(query_ids):
        ...
        if center_available[qrow] and valid_train_ids:
            similarity_to_train = reaction_center_similarity_to_train[qi]
            nk = effective_reaction_neighbours
            nearest = np.lexsort(
                (np.asarray(valid_train_ids, dtype=object), -similarity_to_train)
            )[:nk]
            neighbour_ids = [valid_train_ids[i] for i in nearest]
            nearest_similarity = float(similarity_to_train[nearest[0]])
        else:
            nk = 0
            nearest = np.zeros(0, dtype=np.int64)
            neighbour_ids = []
        ...
        local_center = reaction_center[local_rows].astype(np.float64, copy=False)
        local_intersection = local_center @ local_center.T
        local_mass = local_center.sum(axis=1)
        local_union = local_mass[:, None] + local_mass[None, :] - local_intersection
        reaction_similarity = np.divide(
            local_intersection,
            local_union,
            out=np.zeros_like(local_intersection),
            where=local_union > 0,
        )
        reaction_distance = 1.0 - np.clip(reaction_similarity, 0.0, 1.0)
        np.fill_diagonal(reaction_distance, 0.0)
        local_center_available = center_available[local_rows]
        ...
        reaction_graph = degree_normalized_affinity(
            self_tuning_distance_knn_affinity(
                reaction_distance, available=local_center_available
            )
        )
        ambient_protein_affinity = self_tuning_cosine_knn_affinity(protein_similarity)
        intrinsic_protein_affinity, protein_diffusion_info = one_step_diffusion_affinity(
            ambient_protein_affinity
        )
        protein_graphs = [degree_normalized_affinity(intrinsic_protein_affinity)]
        ...
        protein_transport_measure = (
            ...
            if protein_graphs
            else empirical_measure
        )
        transported_measure = anisotropic_reaction_pushforward(
            prior,
            protein_transport_measure,
            reaction_graph,
            scale=transport_scale,
        ...
        diagnostics.append(
            {
                ...
                "protein_diffusion_edges": int(protein_diffusion_info["edges"]),
                "protein_diffusion_degree_median": float(protein_diffusion_info["degree_median"]),
                "protein_diffusion_degree_max": int(protein_diffusion_info["degree_max"]),
    ...
    summary = {
        "method": "protein_one_step_diffusion_charbonnier_bregman_field_v1",
        "fold": fold,
        ...
        "reaction_chart_rule": "ceil(sqrt(fold_train_reactions_with_center_geometry)) unless explicitly overridden for diagnostics",
        "reaction_geometry": "atom_mapped_reaction_center_tanimoto",
        "reaction_center_dimension": center_dim,
        "protein_geometry": "exact t=1 diffusion distance of the self-tuning ESM-C/observed-CLIP local graph, re-charted by the same intrinsic sqrt(N) sampling rule; compares stationary-measure-weighted Markov neighbourhoods rather than ambient cosine coordinates",
        "flow_strength": 1.0,
        "positive_measure_strength": 0.0 if ablation == "no_observations" else 1.0,
        "positive_measure_transport": "fold-train positive relation restricted to the local chart, symmetrically degree-normalized as A_re/sqrt(d_r d_e), globally normalized to one joint probability measure, then pushed by mass-conserving F0-dependent Charbonnier stochastic kernels on protein and reaction product-edges; no direction-specific conditional normalization",
