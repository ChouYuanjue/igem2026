...
V5 = ROOT / "results/geometric_product_flow_clean_dev_v10"
V5_BASELINE = ROOT / "results/geometric_product_flow_clean_dev_v5"
V8_BASELINE = ROOT / "results/geometric_product_flow_clean_dev_v8"
...
def main() -> None:
    ...
    v5_baseline = pd.read_csv(V5_BASELINE / "development_oof_query_metrics.csv").sort_values(["fold", "query_id"]).reset_index(drop=True)
    v8_baseline = pd.read_csv(V8_BASELINE / "development_oof_query_metrics.csv").sort_values(["fold", "query_id"]).reset_index(drop=True)
    ...
    if not v5[["fold", "query_id"]].equals(v5_baseline[["fold", "query_id"]]):
        raise RuntimeError("v10/v5 query keys differ")
    if not v5[["fold", "query_id"]].equals(v8_baseline[["fold", "query_id"]]):
        raise RuntimeError("v10/v8 query keys differ")
    if not v5[["fold", "query_id"]].equals(bime[["fold", "query_id"]]):
        raise RuntimeError("v10/BiME query keys differ")
    ...
    result = {
        "method": "factor_sum_product_heat_charbonnier_bregman_field_v10",
        ...
        "v5_metrics": metric_map(v5_baseline),
        "v8_metrics": metric_map(v8_baseline),
        "bime_metrics": metric_map(bime),
        "delta_v10_vs_v8": {k: metric_map(v5)[k] - metric_map(v8_baseline)[k] for k in metric_map(v5)},
        "delta_v10_vs_v5": {k: metric_map(v5)[k] - metric_map(v5_baseline)[k] for k in metric_map(v5)},
        "delta_v10_vs_v1": {k: metric_map(v5)[k] - metric_map(v1)[k] for k in metric_map(v5)},
        "delta_v5_vs_bime": {k: metric_map(v5)[k] - metric_map(bime)[k] for k in metric_map(v5)},
        "folds": folds,
        "convergence": convergence,
        "leakage_audit": leakage,
        "paired_bootstrap_v10_vs_v8": bootstrap(v5, v8_baseline),
        "paired_bootstrap_v10_vs_v5": bootstrap(v5, v5_baseline),
        "paired_bootstrap_v5_vs_v1": bootstrap(v5, v1),
        "paired_bootstrap_v5_vs_bime": bootstrap(v5, bime),
        "provenance_sha256": {
            "evaluator": sha256(ROOT / "archive/terpene_screening/lineage/evaluate_geometric_product_flow_clean_dev_v10.py"),
            "multiscale_geometry": sha256(ROOT / "projects/active/terpene_screening/geometry/multiscale.py"),
            "product_field": sha256(ROOT / "projects/active/terpene_screening/geometry/product_field.py"),
            "pair_measure": sha256(ROOT / "projects/active/terpene_screening/geometry/pair_measure.py"),
    ...
    (V5 / "paired_bootstrap_vs_v8.json").write_text(json.dumps(result["paired_bootstrap_v10_vs_v8"], indent=2) + "\n")
    (V5 / "paired_bootstrap_vs_v5.json").write_text(json.dumps(result["paired_bootstrap_v10_vs_v5"], indent=2) + "\n")
