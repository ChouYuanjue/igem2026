...
"""Same-field E2R fibre audit with geometric chart prolongation.
...
from archive.terpene_screening.lineage.evaluate_geometric_product_flow_clean_dev_v8 import (
...
OUT = ROOT / "results/geometric_product_field_bidirectional_audit_v2"
...
def evaluate_fold(
    ...
    for qi, protein in enumerate(query_proteins):
        ...
        # A local chart is a numerical discretisation of one global scalar field,
        # not a hard region in which the model is allowed to change.  Prolong the
        # solved chart deformation back to the complete reaction support through
        # the same intrinsic partition of unity used to restrict positive atoms.
        # This closes the restriction -> solve -> prolongation loop without an
        # E2R-specific score, threshold, or learned interpolation parameter.
        prolongation = _project_reaction_atoms_to_chart(
            all_reaction_rows, reaction_chart, reaction_views, device=device
        )
        chart_delta = flowed[:,0] - prior[:,0]
        full_delta = prolongation @ chart_delta
        flow_scores = full_base + full_delta
        # Preserve the solved nodal values at chart landmarks exactly.  Off-chart
        # values are the geometric partition-of-unity extension.
        flow_scores[reaction_chart] = flowed[:,0]
        ...
        diagnostics.append({
            ...
            "prolongation_offchart_delta_l2":float(np.linalg.norm(full_delta[np.setdiff1d(all_reaction_rows,reaction_chart,assume_unique=False)])),
    ...
    result={
        "method":"same_scalar_product_field_restriction_solve_prolongation_audit_v2",
        "direction":"E2R",
        "role":"bidirectional completeness audit with intrinsic partition-of-unity prolongation on exactly the current R2E clean double-cold pairs; not a comparison to historical E2R splits",
...
def main()->None:
    ap=argparse.ArgumentParser(); ap.add_argument('--fold',type=int,required=True,choices=(0,1,2)); ap.add_argument('--device',default='cuda' if torch.cuda.is_available() else 'cpu'); ap.add_argument('--reaction-chart-k',type=int,default=0); ap.add_argument('--protein-neighbours',type=int,default=0); ap.add_argument('--max-steps',type=int,default=128); ap.add_argument('--max-queries',type=int,default=None); a=ap.parse_args()
    print(json.dumps(evaluate_fold(a.fold,a.device,a.reaction_chart_k,a.protein_neighbours,a.max_steps,a.max_queries),indent=2),flush=True)
