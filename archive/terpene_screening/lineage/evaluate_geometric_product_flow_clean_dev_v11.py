...
OUT = ROOT / "results/geometric_product_flow_clean_dev_v11"
...
def _prolong_protein_chart_delta(
    chart_rows: np.ndarray,
    chart_similarity: np.ndarray,
    chart_delta: np.ndarray,
    *,
    esmc: np.ndarray,
    clip_pt: torch.Tensor,
    clip_lookup: np.ndarray,
    ...
    """Partition-of-unity prolongation of a chart field deformation.

    This is the field-dual of the v8 positive-atom restriction kernel: ESM-C is
    always observed, CLIP refines only pairs for which both structures exist,
    and local bandwidths use the same sqrt(chart-size) self-tuning scale.  The
    chunk size affects only memory use, never the numerical definition.
    """
    chart = np.asarray(chart_rows, dtype=np.int64).reshape(-1)
    delta = np.asarray(chart_delta, dtype=np.float64).reshape(-1)
    if len(chart) != len(delta):
        raise ValueError("chart/delta length mismatch")
    if len(chart) < 2:
        return np.full(full_count, float(delta[0]) if len(delta) else 0.0, dtype=np.float64)
    ...
    scale_k = min(len(chart) - 1, max(1, int(math.ceil(math.sqrt(len(chart))))))
    chart_sigma = np.partition(chart_distance, scale_k - 1, axis=1)[:, scale_k - 1]
    finite_chart = chart_sigma[np.isfinite(chart_sigma) & (chart_sigma > 1e-8)]
    chart_fallback = float(np.median(finite_chart)) if len(finite_chart) else 1.0
    chart_sigma = np.where(
        np.isfinite(chart_sigma) & (chart_sigma > 1e-8), chart_sigma, chart_fallback
    )

    with torch.no_grad():
        ct = F.normalize(
            torch.as_tensor(np.asarray(esmc[chart], dtype=np.float32).copy(), device=device), dim=1
        )
    chart_clip = clip_lookup[chart]
    cj = np.flatnonzero(chart_clip >= 0)
    cp = None
    if len(cj):
        with torch.no_grad():
            cp = F.normalize(
                clip_pt[torch.as_tensor(chart_clip[cj], dtype=torch.long, device=device)].float(), dim=1
            )

    out = np.empty(full_count, dtype=np.float64)
    for start in range(0, full_count, int(chunk_size)):
        stop = min(full_count, start + int(chunk_size))
        rows = np.arange(start, stop, dtype=np.int64)
        with torch.no_grad():
            ot = F.normalize(
                torch.as_tensor(np.asarray(esmc[rows], dtype=np.float32).copy(), device=device), dim=1
            )
            similarity = (ot @ ct.T).cpu().numpy().astype(np.float64)
        obs_clip = clip_lookup[rows]
        oi = np.flatnonzero(obs_clip >= 0)
        if len(oi) and len(cj) and cp is not None:
            with torch.no_grad():
                op = F.normalize(
                    clip_pt[torch.as_tensor(obs_clip[oi], dtype=torch.long, device=device)].float(), dim=1
                )
                structural = (op @ cp.T).cpu().numpy().astype(np.float64)
            similarity[np.ix_(oi, cj)] = 0.5 * (similarity[np.ix_(oi, cj)] + structural)
        distance = np.sqrt(np.maximum(2.0 - 2.0 * np.clip(similarity, -1.0, 1.0), 0.0))
        obs_sigma = np.partition(distance, scale_k - 1, axis=1)[:, scale_k - 1]
        finite_obs = obs_sigma[np.isfinite(obs_sigma) & (obs_sigma > 1e-8)]
        fallback = (
            float(np.median(np.concatenate([finite_chart, finite_obs])))
            if len(finite_obs) else chart_fallback
        )
        obs_sigma = np.where(
            np.isfinite(obs_sigma) & (obs_sigma > 1e-8), obs_sigma, fallback
        )
        denominator = np.maximum(obs_sigma[:, None] * chart_sigma[None, :], 1e-8)
        weights = np.exp(-np.square(distance) / denominator)
        mass = weights.sum(axis=1, keepdims=True)
...
def evaluate_fold(
    ...
    for qi, q in enumerate(query_ids):
        ...
        # The chart is a discretisation of one global field, not a hard region
        # of influence.  Prolong the solved fibre deformation through the same
        # protein partition of unity used to restrict empirical positive atoms.
        chart_delta = flowed[0] - prior[0]
        full_delta = _prolong_protein_chart_delta(
            candidates, protein_similarity, chart_delta, esmc=esmc,
            clip_pt=clip_pt, clip_lookup=clip_lookup, device=device,
            full_count=len(ids),
        )
        flow_scores = full_base + full_delta
        # Nodal chart values are the actual variational solution.
        ...
        diagnostics.append(
            {
                ...
                "prolongation_full_delta_l2": float(np.linalg.norm(full_delta)),
            }
        )
    ...
    summary = {
        "method": "order_symmetric_product_measure_charbonnier_bregman_field_with_partition_prolongation_v11",
        "fold": fold,
        ...
        "protein_domain_rule": "target sqrt(N) high-potential chart; positive atoms restrict and solved field deformation prolongs through the same self-tuning protein partition of unity",
