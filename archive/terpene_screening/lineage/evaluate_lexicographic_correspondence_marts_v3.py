...
CACHE=ROOT/'data/terpene_marts_adaptation';PG=ROOT/'data/terpene_multiresolution_protein_geometry_v4';RG=ROOT/'data/terpene_multiresolution_reaction_geometry_v1';OUT=ROOT/'results/terpene_lexicographic_correspondence_dev_v3';BUD=(3,10,20)
...
def rank_lex(primary,secondary,ids,pos,budgets):
 ...
 while start<len(base):
  ...
  local=np.lexsort((ids_arr[group],secondary[group]))
...
def main():
 ...
 summary={'version':'terpene-lexicographic-correspondence-dev-v3','partition':'development_only','ranking':'machine-stable lexicographic minimization of (Delta_Omega, J_Omega): primary defects within 64 machine eps at the query scale are one numerical level set; J_Omega resolves only that level set; candidate ID breaks any remaining exact tie','interpretation':'correspondence defect is primary; shared-precedent distance only resolves its geometric level sets and can never override a smaller defect','parameter_selection':'none','characteristic_lengths':{'reaction':lr,'protein':le},'all_overlaps_zero':bool(((sp.protein_overlap==0)&(sp.reaction_overlap==0)).all()),'metrics':met.to_dict('records')}
