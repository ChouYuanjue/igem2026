...
import numpy as np
from projects.active.terpene_screening.runtime.reaction_model import FEATURE_NAMES,BASELINE_ANCHOR_INDEX,feature_matrix,structure_configs
ROOT=Path(__file__).resolve().parents[3]

def test_protocol_is_fresh_and_frozen_before_development():
 p=json.loads((ROOT/'configs/retrieval/reaction_model.json').read_text())
...
def test_confirmation_protocol_freezes_selected_config_and_strict_gate_before_reveal():
 p=json.loads((ROOT/'reproducibility/bime_rank/records/UNIFIED_SAFE_SYSTEM_E2R_ANCHORED_LAMBDAMART_V3_CONFIRMATION.json').read_text())
...
def test_confirmation_gate_requires_no_regression_and_material_gain():
 from reproducibility.bime_rank.support.run_unified_safe_system_e2r_anchored_v3_confirmation import gate
...
def test_confirmation_result_passes_and_forbids_same_fold_retuning():
 r=json.loads((ROOT/'configs/retrieval/reaction_model_validation.json').read_text())
...
def test_production_protocol_preserves_scope_and_runtime_gates():
 p=json.loads((ROOT/'configs/retrieval/reaction_ranking.json').read_text())
...
def test_runtime_anchored_order_matches_frozen_positive_rank_transform():
 ...
 ranks=np.stack([__import__('projects.active.terpene_screening.runtime.reaction_model',fromlist=['full_ranks']).full_ranks(S[e]) for e in range(4)],axis=1)
...
def test_runtime_gate_query_selection_is_label_free_and_deterministic():
 from reproducibility.bime_rank.support.benchmark_e2r_anchored_lambdamart_v3_runtime import fixed_query_ids
...
def test_live_manifest_declares_bime_v4_with_confirmed_v3_fallbacks():
 import yaml
 live=yaml.safe_load((ROOT/'configs/production_routes/default.yaml').read_text())
...
def test_e2r_v3_fast_path_scope_is_exact_and_special_requests_fall_back():
 ...
 from projects.active.terpene_screening.runtime import cli as rw
 def args(extra=None):
