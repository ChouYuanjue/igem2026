...
def test_evaluator_is_no_training_and_full_support():
 s=(Path(__file__).resolve().parents[3]/'projects/active/terpene_screening/runtime/reaction_support.py').read_text(); assert 'assert len(common)==11081' in s; assert 'model_training_performed' in s; assert 'evaluate_full_candidate_scores' in s; assert 'dev_pairs.csv' in s
