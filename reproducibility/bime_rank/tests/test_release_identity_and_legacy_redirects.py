...
def test_current_retrieval_status_is_a_compatibility_redirect():
    text = (ROOT / "reproducibility/bime_rank/records/CURRENT_RETRIEVAL_STATUS.md").read_text()
    assert "Compatibility redirect" in text
    assert "canonical.json" in text and "configs/production_routes/default.yaml" in text
...
def test_historical_human_docs_are_redirects_not_current_authority():
    for name in (
        ...
        active = ROOT / "projects/active/terpene_screening" / name
        assert not active.exists()
        text = (ROOT / "reproducibility/bime_rank/records" / name).read_text()
...
def test_current_scorecard_keeps_claim_scopes_separate():
    score = json.loads(
        (ROOT / "reproducibility/bime_rank/records/BIME_RANK_RETRIEVAL_CAPABILITY_SCORECARD_V2.json").read_text()
    )
...
def test_current_model_asset_index_is_route_derived_and_complete():
    ...
    assert model_assets["authority"] == "configs/production_routes/default.yaml"
...
def test_plain_pytest_is_scoped_to_release_maintenance_tests():
    ...
    assert "run_reproduction_tests.py --tier release" in root_readme
    assert "run_reproduction_tests.py --tier extended" in root_readme
...
def test_asset_audit_uses_machine_gates_instead_of_stale_test_counts():
    ...
    assert "run_reproduction_tests.py --tier release|extended" in text
