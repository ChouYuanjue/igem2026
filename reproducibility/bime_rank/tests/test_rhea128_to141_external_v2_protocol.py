...
import pandas as pd
from reproducibility.bime_rank.scripts.prepare_rhea_snapshot_delta_external_benchmark_v2 import build_unambiguous_protein_alias_map,map_to_frozen_universe,build
ROOT=Path(__file__).resolve().parents[3]
P=ROOT/'reproducibility/bime_rank/records/CLEANROOM_R2E_RHEA128_TO141_EXTERNAL_V2.json'
I=ROOT/'reproducibility/bime_rank/records/CLEANROOM_R2E_RHEA128_TO141_EXTERNAL_V1_INVALIDATION.json'
...
def test_builder_manifest_contract_is_compatible_with_full_candidate_evaluator_source():
    source=(ROOT/'reproducibility/bime_rank/scripts/prepare_rhea_snapshot_delta_external_benchmark_v2.py').read_text()
    assert "'valid':True" in source
    assert "'violations':[]" in source



def test_release_rebuilder_uses_tracked_clean_boundary_as_alignment_witness():
    from reproducibility.bime_rank.scripts.rebuild_rhea128_to141_strict_support_v2 import compact_alignment_witness
    clean=pd.DataFrame([{'protein_id':'SEEN','reaction_id':'RHEA:9'},{'protein_id':'SEEN','reaction_id':'RHEA:9'}])
    witness=compact_alignment_witness(clean)
    assert witness.to_dict('records')==[{'protein_source_id':'SEEN','reaction_source_id':'RHEA:9'}]

def test_frozen_protocol_file_hash_remains_immutable():
    import hashlib
    digest=hashlib.sha256(P.read_bytes()).hexdigest()
    result=json.loads((ROOT/'reproducibility/bime_rank/records/CLEANROOM_R2E_RHEA128_TO141_EXTERNAL_V2_RESULT.json').read_text())
    assert digest==result['frozen_protocol_sha256']=='bd372107c2d6a5c47e42fb371610d60fc75a2c810af6dd9b261353a3ea12ce19'
