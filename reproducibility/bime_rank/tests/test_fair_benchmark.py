...
from projects.active.terpene_screening.runtime.evaluation_metrics import audit_exact_overlap, evaluate_ranking_frame
...
def test_common_metrics_match_rank_semantics() -> None:
    ...
    assert summary["mean_positive_rank"] == 2.0
    assert summary["mean_positive_reciprocal_rank"] == pytest.approx((0.375 + 1.0) / 2.0)
    assert "precision_at_50" in summary
